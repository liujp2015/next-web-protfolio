...
用于 Typescript 或 ES6+的类验证，基于[validator.js](https://github.com/chriso/validator.js)
...
`validate`函数的第二个参数是一个选项对象，尽量设置`forbidNonWhitelisted`为`true`以避免 unkown 对象的输入验证
...
在 http 响应中我们一般不想在错误中暴露`target`，那么就可以如下方式禁用它
...
由于装饰器的性质，必须使用`new class()`语法实例化待验证的对象。如果你使用了 class-validator 装饰器定义了类，并且想要验证普通的 JS 对象（文本对象或 JSON.parse 返回），则需要将其转换为类实例（例如，使用[class-transformer](https://github.com/pleerock/class-transformer)或仅使用[class-transformer-validator](https://github.com/19majkel94/class-transformer-validator)扩展可以为您完成此任务。
...
类转换器的作用是将普通的 javascript 对象转换成类对象。我们通过 api 端点或者 json 文件访问所得的是普通的 json 文本，一般我们通过`JSON.parse`把其转换成普通的 javascript 对象，但是有时候我们想让它变成一个类的对象而不是普通的 javascript 对象。比如用`class-validator`来验证从后端 api 获取的 json 字符串时，我们就需要自动把 json 转为待验证类的对象而不是一个 js 对象。

例如我们现在可以读取远程 api 的一个`users.json`的内容如下
...
现在你可以获取`users[0].firstname`但是由于你获取的是普通的 js 对象而非`User`类的对象，所以你无法调用`users[0].getName()`方法，而**class-transformer**就是为了把普通的 js 对象按你的需求转换成类对象而生的。
...
2. 安装`reflect-metadata`
   ...
   > reflect-metadata 是必须的，具体使用请看[这篇博文](./2022-06-23-ts-decorator.md)
...
> 转换后可以使用`JSON.stringify`再转成普通的 json 文本
...
如果 json 文本是个对象数组请使用`deserializeArray`方法
...
由于现在 Typescript 对反射还没有非常好的支持，所以你需要使用`@Type`装饰器来隐式地指定属性所属的类
...
一个嵌套的子类型也可以匹配多个类型，这可以通过判断器实现。判断器需要指定一个` property`，而被转换 js 对象中的嵌套对象的也必须拥有与`property`相同的一个字段，并把值设置为需要转换的子类型的名称。判断器还需要指定所有的子类型值以及其名称，具体示例如下
...
添加`@Expose`装饰器即可公开 getter 和方法的返回值
...
如果要构建具有不同版本的 API，则 class-transformer 具有非常有用的工具。您可以控制应在哪个版本中公开或排除模型的哪些属性。示例
...
有时，您的 JavaScript 对象中有一个以字符串格式接收的 Date。您想从中创建一个真正的 javascript Date 对象。您只需将 Date 对象传递给`@Type`装饰器即可完成此操作：
...
上述装饰器接受一个可选参数：`ClassTransformOptions`-转换选项，例如 groups, version, name，示例：
...
`user`对象将包含 firstname,latstname 和 email

#### 使用泛型

由于目前 Typescript 对反射的支持还没有完善，所以只能使用其它替代方案，具体可以查看[这个例子](https://github.com/pleerock/class-transformer/tree/master/sample/sample4-generics)
