import { Card } from "@/components/tailwindplus/Card";
import { SimpleLayout } from "@/components/tailwindplus/SimpleLayout";
import { formatDate } from "@/lib/formatDate";
import { queryPostPaginate } from "../_actions/post";
import { PaginationDemo } from "../_components/pagination";

function Article({ article }) {
  return (
    <article className="md:grid md:grid-cols-4 md:items-baseline">
      <Card className="md:col-span-3 overflow-hidden">
        <Card.Title href={`/blog/${article.id}`}>{article.title}</Card.Title>
        <Card.Eyebrow
          as="time"
          dateTime={article.createdAt}
          className="md:hidden"
          decorate
        >
          {formatDate(article.createdAt)}
        </Card.Eyebrow>
        <Card.Description className="truncate ">
          {article.body}
        </Card.Description>
        <Card.Cta>Read article</Card.Cta>
      </Card>
      <Card.Eyebrow
        as="time"
        dateTime={article.createdAt}
        className="mt-1 hidden md:block"
      >
        {article.id} {formatDate(article.createdAt)}
      </Card.Eyebrow>
    </article>
  );
}

export const metadata = {
  title: "Articles",
  description:
    "All of my long-form thoughts on programming, leadership, product design, and more, collected in chronological order.",
};

export default async function ArticlesIndex({ searchParams }) {
  const { page, limit = 8 } = await searchParams;
  const articles = await queryPostPaginate({
    page: parseInt(page ? page : 1),
    limit,
  });
  return (
    <SimpleLayout
      title="Writing on software design, company building, and the aerospace industry."
      intro="All of my long-form thoughts on programming, leadership, product design, and more, collected in chronological order."
    >
      <div className="md:border-l md:border-zinc-100 md:pl-6 md:dark:border-zinc-700/40">
        <div className="flex max-w-3xl flex-col space-y-16">
          {articles.items.map((article) => (
            <Article key={article.id} article={article} />
          ))}
        </div>
      </div>
      <PaginationDemo
        totalPages={articles.meta.totalPages}
        currentPage={articles.meta.currentPage}
      />
    </SimpleLayout>
  );
}
